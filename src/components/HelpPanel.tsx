    ...
    <div className="help">
      ...
      <section>
        ...
        <ul>
          ...
          <li>
            <b>Sharipov-Fahrenbach-Zipp 2005</b> (J. Vac. Sci. Technol. A 23,
            1331):{" "}
            <a
              href="https://doi.org/10.1116/1.1991882"
              target="_blank"
              rel="noreferrer"
            >
              DOI
            </a>
            . Строгая кинетическая модель на основе BGK-уравнения: четыре
            вспомогательные задачи (продольные и поперечные Куэтт/Пуазейль в
            сечении канавки) решаются раздельно и суперпонируются (Ур. 46, 52):{" "}
            <code>
              S_max = (2πR·U·ℓ²/ℓ_z)·[G_z^C − (ℓ_z/ℓ_x)·G_x^C]·sinα·cosα
            </code>
            . Коэффициенты <code>G_z^C</code>, <code>G_x^C</code>,{" "}
            <code>G_z^P</code>, <code>G_x^P</code> табулируются по параметру
            разрежения <code>δ = Pℓ/(μ·v_m)</code>. Мы используем только
            свободно-молекулярный предел (δ→0: <code>G_z^C ≈ 1.0</code>,{" "}
            <code>G_x^C ≈ 0.35</code>, <code>G_x^P/G_z^P ≈ 0.4</code>) —
            полный расчёт требует таблиц BGK-решений. Даёт значения порядка
            Sickafus, но без зависимости от sinθ·cosθ в h_eff. Статья также
            содержит экспериментальные данные Pfeiffer Vacuum для
            K₀(δ_f) и τ(δ_f) по Ar, N₂, He — Figs. 7–8.
          </li>
          <li>
            <b>Sawada-Sugiyama 1999</b> (J. Vac. Sci. Technol. A 17, 2069): в
            свободно-молекулярном пределе балансом импульса в канавке (Ур. 5–9
            статьи) получаем{" "}
            <code>ln K_fm = u·cosα·L / (v_m · h_Sawada)</code>, где{" "}
            <code>h_Sawada = h·[1 + w/(s·σ)]</code>,{" "}
            <code>σ = (h+δ)/δ</code>. Это точно воспроизводит плато K ≈ 100 для
            экспериментального ротора из Table I статьи (D=50, L=40, h=1 мм,
            δ=0.2 мм, α=14.28°, 300 об/с). Переход в вязкий режим учитывается
            взвешенной комбинацией κ·q_free + (1−κ)·q_slip (Ур. 10–11), где
            κ = exp(−8·c₂·y_m/λ) — доля «свободных» молекул; мы применяем это
            через отдельную функцию K(P) в модели кривой S(P_вх).
          </li>
        </ul>
        <p className="muted small">
      ...
      <section>
        ...
        <p>
          <b>Посекционное применение f(Kn) для совмещённого режима</b>. В
          совмещённом TMP турбо-ступень находится на входе (P_вх, крупный
          осевой зазор ~0.5–2 мм) и при росте P_вх входит в переходный режим
          раньше, чем Holweck-ступень (глубина канавки 0.2–0.5 мм), которая
          работает при P_mid ≈ P_вых/K_hol. Поэтому каждая секция имеет
          собственный фактор <code>f_vi</code>:
        </p>
        <div className="eq">
          f_vi,турбо = sFrac(h_turbo, √(P_вх·P_mid))
          <br />
          f_vi,holweck = sFrac(h_holweck, √(P_mid·P_вых))
          <br />
          P_mid ≈ max(P_вх, P_вых / K_hol)
        </div>
        <p className="muted small">
          Итоговый фактор — минимум из двух секций (узкое место). Это
          обеспечивает падение S при росте P_вых: когда форвакуум плохой,
          P_mid растёт, и Holweck-секция входит в переходный режим даже при
          малых P_вх. Физическое обоснование: Sun et al., <i>Vacuum</i> 225
          (2024) 113201 — непрерывная модель TMP во всех режимах потока;
          Amoli & Ebrahimi, <i>Vacuum</i> 75 (2004) 361 — континуум-модель
          турбомолекулярной ступени, показывающая падение W (коэффициента
          пропускания) при переходе Kn ≈ 1.
        </p>
      </section>

      <section>
        <h3>
          6d. Рекомендации по выбору методики (по результатам валидации)
        </h3>
        <p>
          По результатам прогонки всех 11 пресетов (5 Kruger + HiPace-turbo + 3
          Holweck + Sawada-1999 + 2 совмещённых) получены следующие
          систематические закономерности.
        </p>
        <h4>Турбо-ступень — K</h4>
        <table className="cmp-table">
          <thead>
            <tr>
              <th>Метод</th>
              <th>Kruger-table (5 шт)</th>
              <th>Особенности</th>
              <th>Рекомендация</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>Kruger 1960 fit</td>
              <td>ΔK = −9…−34%</td>
              <td>Прямая подгонка Kruger tables</td>
              <td className="rec-ok">По умолчанию</td>
            </tr>
            <tr>
              <td>Sawada-Hirata 1974</td>
              <td>Идентично Kruger (ΔK = −9…−34%)</td>
              <td>Оригинальная функция W(C,α,s/b)</td>
              <td className="rec-ok">Эквивалентно</td>
            </tr>
            <tr>
              <td>Bernhardt 1983</td>
              <td>ΔK = −69…−93%</td>
              <td>Упрощённая 2D-модель, занижает K</td>
              <td className="rec-warn">Не использовать для K</td>
            </tr>
          </tbody>
        </table>
        <p className="muted small">
          Вывод: для турбо используйте <b>Kruger 1960 fit</b> (или
          эквивалентный Sawada-Hirata). Bernhardt подходит только для грубых
          оценок W при α &gt; 30°.
        </p>

        <h4>Holweck-ступень — K</h4>
        <table className="cmp-table">
          <thead>
            <tr>
              <th>Метод</th>
              <th>Sawada-1999 (D=50, L=40)</th>
              <th>Giors-2006 (DSMC, L=60, h=0.3)</th>
              <th>HiPace-combined</th>
              <th>Диапазон применимости</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>Sickafus / Jousten</td>
              <td>−97%</td>
              <td>+374%</td>
              <td>−40%</td>
              <td className="rec-ok">
                Универсальный выбор, ±5× по K. Лучше для больших D, умеренных h
              </td>
            </tr>
            <tr>
              <td>Sharipov 2005 (BGK, fm-предел)</td>
              <td>−98%</td>
              <td>+278%</td>
              <td>≈ Sickafus</td>
              <td className="rec-ok">
                Слегка консервативнее Sickafus. Для кинетического исследования
              </td>
            </tr>
            <tr>
              <td>Sawada 1999 (h_eff)</td>
              <td className="rec-ok">−32%</td>
              <td>+5·10⁹% ✗</td>
              <td>−100% ✗</td>
              <td>
                Оптимально для геометрии Sawada (L/D ~ 0.8, умеренные h/δ).
                Переоценивает K для длинных глубоких канавок.
              </td>
            </tr>
            <tr>
              <td>Gaede / Boulon-Audi</td>
              <td>+5000…+10⁵%</td>
              <td>+10¹²%</td>
              <td>+10¹⁷%</td>
              <td className="rec-warn">
                Только L·sinα/h &lt; 1 (очень короткие ступени). Для
                реалистичных насосов взрываются.
              </td>
            </tr>
          </tbody>
        </table>
        <p>
          <b>Выбор по Kn-режиму и геометрии</b>:
        </p>
        <ul>
          <li>
            <b>Короткая Holweck-ступень</b> (L/D &lt; 1, умеренные h ≈ 1 мм):{" "}
            <b>Sawada 1999</b> — ±30% по K. Первый выбор для демонстрационных
            и малых ступеней.
          </li>
          <li>
            <b>Длинная Holweck-ступень коммерческого TMP</b> (L = 50–80 мм,
            h = 0.2–0.5 мм):{" "}
            <b>Sickafus / Jousten</b> — порядок величины по K, разумный S.
            Первый выбор для HiPace / TURBOVAC / MAG W.
          </li>
          <li>
            <b>Кинетическое исследование в переходном режиме</b>: Sharipov 2005
            (BGK) даёт близкие значения и физически более строг. Разница со
            Sickafus в пределах ±30%.
          </li>
          <li>
            <b>Никогда</b> не используйте Gaede / Boulon-Audi для L/h &gt; 20
            — формулы расходятся экспоненциально и дают K ~ 10¹²…10²⁰.
          </li>
        </ul>
        <h4>Комбинированный режим (турбо + Holweck)</h4>
        <p>
          Рекомендуемое сочетание: <b>Kruger (турбо) + Sickafus (Holweck)</b>.
          Это даёт в пределах −40% по K для HiPace 300 combined, в пределах
          ±30% по S. Для более точной оценки при заданной геометрии
          сравнивайте результаты всех пяти Holweck-методов и выбирайте
          ближайший к эталонной S(P)-кривой пресета.
        </p>
        <h4>Мощность</h4>
        <p>
          Расчётная мощность — только газодинамический сдвиг. Для коммерческих
          насосов следует добавлять 30–80 Вт подшипников + 30–50% потерь
          двигателя. Не используйте расчётную P для сравнения с полной
          потребляемой мощностью datasheet (см. раздел 7).
        </p>
      </section>
...
statorAngleDeg,statorCount,statorHeight
      ...
      <section>
        ...
        <ul>
          ...
          <li>
            Мощность — только газодинамические потери (сдвиг на поверхности
            лопаток/ротора в слое газа, <code>τ ≈ ρ·v_m·u/8</code>), с
            использованием давления на выходе насоса, а не локально
            сжатого давления. В расчёт не входят: трение в подшипниках
            (15–80 Вт), потери двигателя (30–50%), теплоотвод. Датасетовые
            значения «нагрузки» 150–300 Вт обычно измеряются при
            максимальном расходе (P_вых ≈ 100–1500 Па) и включают все эти
            источники. Наша оценка газовой мощности даёт порядок величины
            для этой компоненты (30–200 Вт при рабочем P_вых), но
            систематически занижает общую потребляемую мощность.
          </li>
        </ul>
      </section>
    </div>
