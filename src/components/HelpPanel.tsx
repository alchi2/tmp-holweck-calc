export function HelpPanel() {
  return (
    <div className="help">
      ...
      <section>
        <h3>6c. Модель кривой S(P_вх)</h3>
        <p>
          Datasheet-кривые TMP/Holweck показывают характерный «плоский»
          молекулярный участок в середине и спады по обе стороны. Мы их
          воспроизводим произведением двух поправок к S_max:
        </p>
        <ul>
          <li>
            <b>Компрессионный предел (слева)</b>:{" "}
            <code>stall = max(0, 1 − P_вых/(K·P_вх))</code>. Ниже P_вых/K насос
            физически не может удержать входное давление — газ течёт обратно
            из форвакуума.
          </li>
          <li>
            <b>Переход в вязкий режим (справа)</b>:{" "}
            <code>viscous = 1/(1 + (P_вх/P_knee)³)</code>, где P_knee —
            давление, при котором число Кнудсена Kn = λ/L ≈ 1. В воздухе при
            293 K λ·P = 6.6·10⁻³ м·Па, поэтому P_knee = 6.6·10⁻³/L_char. В
            качестве L_char берётся минимальный канал в тракте переноса
            (осевой зазор для турбо или глубина канавки для Holweck).
          </li>
        </ul>
        <p className="muted small">
          Уплотнительные зазоры (tip clearance, radial clearance) не входят в
          L_char — они уменьшают обратный поток, но не определяют, где Kn→1
          в основном канале переноса газа. Реальные насосы показывают спад
          при P_вх ≈ P_knee × (0.5–3), степень спада 2–4 порядка.
        </p>
      </section>
